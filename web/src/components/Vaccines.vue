  ...
  <el-card>
    ...
    <el-row :gutter="12">
      <el-col :span="6">
        <el-input clearable
                  @clear="listPage"
                  v-model="avcname"
                  placeholder="请输入疫苗名称查询"></el-input>
      </el-col>
      <el-col :span="6">
    ...
    <el-table :data="tableData">
      ...
      <el-table-column prop="manufactor"
                       label="生产厂家"></el-table-column>
      <el-table-column prop="mdate"
                       label="生产日期"></el-table-column>
      <el-table-column prop="price"
                       label="单价"></el-table-column>
      <el-table-column prop="instructions"
    ...
    <el-dialog :visible.sync="addFlag"
      ...
      <el-form>
        <el-form-item label="疫苗名称">
          <el-input v-model="vaccines.avcname"
                    placeholder="请输入疫苗名称"></el-input>
        </el-form-item>
        <el-form-item label="生产厂家">
          <el-input v-model="vaccines.manufactor"
                    placeholder="请输入生产厂家"></el-input>
        </el-form-item>
        <el-form-item label="生产日期">
          <el-date-picker v-model="vaccines.mdate"
                          type="date"
        ...
        <el-form-item label="单价">
          <el-input v-model="vaccines.price"
                    placeholder="请输入单价"></el-input>
        </el-form-item>
        <el-form-item label="说明">
          <el-input v-model="vaccines.instructions"
                    type="textarea"
...
export default {
  ...
  methods: {
    ...
    save () {
      this.$http.post('http://127.0.0.1//vaccines/save', this.vaccines).then(res => {
    ...
    update () {
      this.$http.post('http://127.0.0.1//vaccines/update', this.vaccines).then(res => {
        if (res.data.code == 200) {
          this.$message({
            message: res.data.message,
            type: 'success'
    ...
    remove (row) {
      this.$http.post('http://127.0.0.1//vaccines/remove?avcid=' + row.avcid).then(res => {
        if (res.data.code == 200) {
          this.$message({
            message: res.data.message,
            type: 'success'
