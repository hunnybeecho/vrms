    ...
    <el-row :gutter="12">
      <el-col :span="6">
        <el-input clearable
                  @clear="listPage"
                  v-model="username"
                  placeholder="请输入管理员名称查询"></el-input>
      </el-col>
      <el-col :span="6">
    ...
    <el-dialog :visible.sync="addFlag"
      ...
      <el-form>
        <el-form-item label="管理员名称">
          <el-input v-model="administrators.username"
                    placeholder="请输入管理员名称"></el-input>
        </el-form-item>
        <el-form-item label="密码">
          <el-input v-model="administrators.password"
                    placeholder="请输入密码"
                    type="password"></el-input>
        </el-form-item>
        <el-form-item label="头像地址">
          <el-input v-model="administrators.images"
                    placeholder="请输入头像地址"
...
export default {
  ...
  methods: {
    ...
    save () {
      this.$http.post('http://127.0.0.1//administrators/save', this.administrators).then(res => {
    ...
    update () {
      this.$http.post('http://127.0.0.1//administrators/update', this.administrators).then(res => {
        if (res.data.code == 200) {
          this.$message({
            message: res.data.message,
            type: 'success'
    ...
    remove (row) {
      this.$http.post('http://127.0.0.1//administrators/remove?aid=' + row.aid).then(res => {
        if (res.data.code == 200) {
          this.$message({
            message: res.data.message,
            type: 'success'
    ...
    changeStats (row) {
      ...
      this.administrators = row;
      this.$http.post('http://127.0.0.1/administrators/changeStats', this.administrators).then(res => {
        console.info(res);
        if (res.data.code == 200) {
          this.$message({
            message: res.data.message,
    ...
    listPage () { // 定义分页方法
      this.$http.get('http://127.0.0.1/administrators/listPage', {
        params: {
